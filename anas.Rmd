...
round(sum(is.na(py$DER))/dim(p"y)[1]*100)
...
In this analysis, which is the main one in the paper, we declare all F categories go to silence
...
In this alternative analysis, we declare all F categories separately. This should lead to higher false alarms (since F now counts as speech) and potentially higher confusion.


```{r mean.metrics-far}
read.csv(paste0(thisdir,"/gold/mapped/mapped_far/diaer__report.csv"))->py
dim(py) #873 clips
summary(py)
...
In this alternative analysis, all the F and N categories map to the same (CHN and CHF go to CH) ** NOT DONE YET**

```{r mean.metrics-joint, eval=F}
length(py$false.alarm..) #total N of clips
summary(py$false.alarm..)
