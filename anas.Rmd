...
spreadsheet_tsi = read.csv(paste0(thisdir,"/tsi_key_info.csv"), header=TRUE, sep = ",")
...
```{r precision}
prop_cat=data.frame(apply(ol,2,dodiv)*100) #generates precision because columns
colSums(prop_cat)
stack(ol)->stol
...
cvtc$child[cvtc$cor=="tsi"]<-substr(cvtc[cvtc$cor=="tsi"]$filename,1,3)
