...
read_its <- function(data_folder, gold_folder) {
  ...
  data <- data %>% filter(spkr == "CHN" | spkr == "CHF" | spkr == "MAN" | spkr == "FAN" | spkr == "CXN") #ac added CHF for CVC/CTC lena-like counts  # columns to paste together
...
read_rttm <- function(data_folder) {
  ...
  cond1_all = prev_spkr == 'CHI' & (next_spkr == 'MAL' | next_spkr == 'FEM')
  cond2_all = (prev_spkr == 'MAL' | prev_spkr == 'FEM') & next_spkr == 'CHI'
  data$adult_chi_swipe_all = cond1_all | cond2_all
  data$turn_taking_all = data$adult_chi_swipe_all & data$less_than_5
  data = data[,c(1,2,3,11,12,13,14, 15, 16,4,5,6,7,8,9,10)]
  return(data)
}

get_stats_gold <- function(gold_data){
  CC = gold_data %>%
    filter(mapped_speaker_type == 'CHI') %>%
  ...
  #ac added whole block above
    ...
    CVC = gold_data %>%
    filter(mapped_speaker_type == 'CHI', tier_subtype == 'C' | tier_subtype == 'N' | tier_subtype == "W") %>%
    dplyr::group_by(filename) %>%
  ...
  CNVC = gold_data %>%
    filter(mapped_speaker_type == 'CHI', tier_subtype == 'L' | tier_subtype == 'U' | tier_subtype == 'Y') %>%
    dplyr::group_by(filename) %>%
...
write.table(format(child,digits=3,trim=TRUE), sep=' ', quote=FALSE, file=paste(output_folder, "key_child_voc_child_level.csv", sep="/"), row.names=FALSE)
