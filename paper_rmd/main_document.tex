...
\caption{\label{tab:tabprops}Number of frames, percentage of frames, and number of minutes attributed to each category by the human annotators.}
...
\item
  omitting OLN as a speaker category, since it is not conceptually identical to the overlap derived from humans' annotating different speaker categories; in this case, LENA\textsuperscript{®}'s OLN labels would be mapped post hoc to \enquote{Other}, as would the regions of overlap in the human annotation.\footnote{Please note that distinguishing the \enquote{near} and \enquote{far} categories cannot be used in the present analysis, which requires a direct one-to-one matching across the system labels and the gold labels. The division between \enquote{near} and \enquote{far} in LENA\textsuperscript{®} is done by comparing the fit to a given category and silence with a likelihood ratio, which one could not have human annotators perform.}
\end{itemize}

The analysis that yields the best LENA\textsuperscript{®} performance (Table 5) focuses on the speaker categories while mapping electronic voices and overlap in the human annotation onto Other, so that the categories considered in the human annotation are FEM, MAL, CHI, OCH, alongside using only CHN, FAN, MAN, and CXN as speakers in the LENA\textsuperscript{®} annotation, (with all \enquote{far} categories, TVN, and OLN all mapped onto Other; see Tables 1 and 3). Calculated in this way, LENA\textsuperscript{®}'s \emph{false alarm} rate (i.e., tagging a speech category when there was none) and \emph{confusion} rate (i.e., providing the wrong label) were lowest and close to the single digits. The \emph{miss} rate (i.e., the system returns a judgment that no sound label is activated) is double that found in the other analysis situations, but this increase in misses is made up for by lower error rates in the other two. The second-best performing case, electronic voices in the human annotation are still mapped onto Other but Overlap is not, so that the human categories considered were CHI, FEM, MAL, OCH, and overlap; and the LENA\textsuperscript{®} categories considered were CHN, FAN, MAN, CXN, and OLN (with all \enquote{far} classes and TVN mapped onto Other). The worst performance arose when we allowed the electronic voices segmented by human annotators, and LENA\textsuperscript{®}'s TVN speaker categories to be included in the evaluation (rather than mapping them all to Other), such that the human categories considered were CHI, FEM, MAL, OCH, overlap, and electronic; and the LENA\textsuperscript{®} categories considered were CHN, FAN, MAN, CXN, OLN, and TVN. It is likely that these differences are partially due to the fact that OLN and TVN are not defined similarly across the system and human annotators.
...
\toprule
 & \multicolumn{4}{c}{Overall} & \multicolumn{4}{c}{Mean} & \multicolumn{4}{c}{Median} \\
\cmidrule(r){2-5} \cmidrule(r){6-9} \cmidrule(r){10-13}
 & \multicolumn{1}{c}{FAR} & \multicolumn{1}{c}{MR} & \multicolumn{1}{c}{CR} & \multicolumn{1}{c}{IDER} & \multicolumn{1}{c}{FAR} & \multicolumn{1}{c}{MR} & \multicolumn{1}{c}{CR} & \multicolumn{1}{c}{IDER} & \multicolumn{1}{c}{FAR} & \multicolumn{1}{c}{MR} & \multicolumn{1}{c}{CR} & \multicolumn{1}{c}{IDER}\\
\midrule
...
We extracted Child Vocalizaton Counts from LENA\textsuperscript{®}'s output and contrasted them with clip-level counts of linguistic vocalizations by the key child in the human annotation.\footnote{In a previous version of this analysis, we had calculated CVC as the number of CHN segments in LENA\textsuperscript{®}, and the number of vocalizations classified as linguistic by human annotators. Further inspection of LENA\textsuperscript{®} documentation revealed this decision was incorrect, since LENA\textsuperscript{®} counts linguistic utterances (and not non-linguistic vocalizations); it can count several of these within the same CHN segment; and it even includes such utterances when part of a CHF segment. Given the inaccuracy of CHF, the latter decision seems potentially problematic. The same issue affected our CTC analyses. We now present in the main paper analyses that respect LENA\textsuperscript{®}'s reported CVC and CTC, because that is what our colleagues are using. In Supplementary Materials, we show results of the correlations and error rate analyses when CVCs are calculated as the number of CHN/CHI segments instead, which are identical, and CTC derived from CHN/CHI segments, which yield slightly worse results than those reported on here.} As shown in Figure 4, there is a strong association between clip-level counts estimated via the LENA\textsuperscript{®} system and those found in the human annotations: the Pearson correlation between the two was r = 0.76 (p = 0.00) when all clips were taken into account, and r = 0.65 (p = 0.00) when only clips with some child speech (i.e., excluding clips with 0 counts in both LENA\textsuperscript{®} and human annotations) were considered (see Table 6 for correlation results of CVC, CTC, and AWC analyses). This suggests that the LENA\textsuperscript{®} system captures differences in terms of number of child vocalizations across clips rather well.
...
The error rates are reported on in Table 7. Generally speaking, CVC error rates suggested that LENA\textsuperscript{®} tends to underestimate vocalization counts, particularly when only clips with some speech are considered. This understimation, however, is not systematic, and cumulating errors using the absolute relative error rate leads to an estimation of 75.55\% devition from the actual counts.
