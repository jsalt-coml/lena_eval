...
median of 6\%, whereas the miss rate had a median of 33\%. The confusion
...
considered as a false alarm). The confusion rate was very low, with a
median of 9\%. These three metrics are added together to yield a median
diarization error rate over the clips that had some speech of 80\%.
...
total median diarization error of 81\%. Performance likely degrades
...
TVN and OLN), the overall relative diarization error rate is about 80\%,
due mainly to missing speech (33\%), with false alarms (6\%) and
confusion between talker categories (9\%) constituting a relatively
...
Precision for CHN comes in second place, at 49\%; thus, fewer than half
...
child, 18\% was captured by LENA\textsuperscript{TM}'s CXN category and
23\% by its OLN tag, with the remainder spread out across several
...
contains a further 18\% of the key child's speech. Moreover, as we saw
...
LENA\textsuperscript{TM} as OLN (mean across MAN and CXN 35\%) or
...
found for CVC: the Pearson correlation between the two was r = 0.59 (p =
0.00) when all clips were taken into account, and r = 0.45 (p = 0.00)
when only clips with some child speech (i.e., excluding 381 clips with 0
