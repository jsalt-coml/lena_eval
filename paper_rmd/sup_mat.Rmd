...
rownames(allpearsons)<-c("CVC","CHN counts","CTC","CTC all CHN","AWC","AWC no French")
colnames(allpearsons)<-c("N clips","r all","N","r")

aller=matrix(nrow=6,ncol=12) #error rates
rownames(aller)<-c("CVC","CHN counts","CTC","CTC all CHN","AWC","AWC no French")
...
cor.chnc.all= cor.test(cvtc$lena_CHN_count,cvtc$gold_CHI_count)
cor.chnc.noZeros= cor.test(cvtc$lena_CHN_count[cvtc$lena_CHN_count>0 & cvtc$gold_CHI_count>0],cvtc$gold_CHI_count[cvtc$lena_CHN_count>0 & cvtc$gold_CHI_count>0])

allpearsons["CHN counts","N clips"]<-round(cor.cvc.all$parameter)
allpearsons["CHN counts","r all"]<-round(cor.cvc.all$estimate,3)
allpearsons["CHN counts","N"]<-round(cor.cvc.noZeros$parameter)
allpearsons["CHN counts","r"]<-round(cor.cvc.noZeros$estimate,3)

aer=(cvtc$lena_CHN_count-cvtc$gold_CHI_count)
aer_no_zeros=(cvtc$lena_CHN_count[cvtc$lena_CHN_count>0 & cvtc$gold_CHI_count > 0]-cvtc$gold_CHI_count[cvtc$lena_CHN_count>0 & cvtc$gold_CHI_count > 0])
no_human_zeros=cvtc[ cvtc$gold_CHI_count>0,]
rer=(no_human_zeros$lena_CHN_count-no_human_zeros$gold_CHI_count)/no_human_zeros$gold_CV_count*100
arer=abs(no_human_zeros$lena_CHN_count-no_human_zeros$gold_CHI_count)/no_human_zeros$gold_CV_count*100
aer_cvc <-(cvtc$lena_CHN_count-cvtc$gold_CHI_count)
aer_nozeros_cvc <-(cvtc$lena_CHN_count[cvtc$lena_CHN_count>0 & cvtc$gold_CHI_count > 0]-cvtc$gold_CHI_count[cvtc$lena_CHN_count>0 & cvtc$gold_CHI_count > 0])


aller["CHN counts",]<-cbind(mean(aer),min(aer),max(aer),
                     mean(aer_no_zeros),min(aer_no_zeros),max(aer_no_zeros),
                     mean(rer),min(rer),max(rer),
                     mean(arer),min(arer),max(arer)
                     )
...
allpearsons["AWC no French","N clips"]<-round(cor.noFrench.all$parameter)
allpearsons["AWC no French","r all"]<-round(cor.noFrench.all$estimate,3)
allpearsons["AWC no French","N"]<-round(cor.noFrench.noZeros$parameter)
allpearsons["AWC no French","r"]<-round(cor.noFrench.noZeros$estimate,3)
...
aller["AWC no French",]<-cbind(mean(aer),min(aer),max(aer),
                     mean(aer_no_zeros),min(aer_no_zeros),max(aer_no_zeros),
                     mean(rer),min(rer),max(rer),
                     mean(arer),min(arer),max(arer)
                     )
...
myrange=range(cvtc[,c("gold_CHI_count","lena_CHN_count")],na.rm=T)
plot(gold_CHI_count~lena_CHN_count,data=cvtc,pch=20,col=alpha("black",.2),xlab="LENA CHN counts",ylab="Human CHI counts",xlim=myrange,ylim=myrange)
abline(lm(gold_CHI_count~lena_CHN_count,data=cvtc))
abline(lm(gold_CHI_count~lena_CHN_count,data=cvtc,subset=c(cvtc$lena_CHN_count>0 & cvtc$gold_CHI_count>0)),lty=2)
