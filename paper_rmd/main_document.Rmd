...
This paper was written using RMarkDown [@baumer2014r] in R [@team2013r] running on Rstudio [@rstudio]. It can be downloaded and reproduced using the data also available from the Open Science Framework, https://osf.io/zdg6s. These online supplementary materials also include a document with additional analyses. 
...
<!-- eb: some suspicious coincidences with identical values in the paragraph above..math doesn't seem to add  up-->
<!-- crazy proposal to reduce readers working memory: calling things FEM-h FEM-s, MAL-h, MAL-s, CHI-h, CHI-s, and OCH-h, OCH-s, etc throughout? otherwise readers will be confused i fear-->
...
<!-- eb: FAN/FEM not darkest cell above, this may change again; check text vs. data again later. one val not rendering at all... -->
<!-- eb: reevaluate the text in following 2 paras once data final-->
...
<!-- eb: same need to reword as previous section given updated results ac2eb I think it's ok, don't you?-->
...
#### Are there differences in false alarm, miss, and confusion rates as a function of corpus and child age? **AC stopped here!!**

We predicted false alarm, miss, and confusion rates in clips where there was some speech according to the human annotator when, with all "Far" categories, TV, and overlap mapped onto Other (which yielded the best results in Section "False alarms, misses, confusion" above.) Our predictors were corpus, child age, and their interaction as fixed effects, and child ID as a random effect. We followed up with a Type III ANOVA to assess significance. Across all analyses, corpus, child age, and their interaction were never significant, with the exception of confusion, where the interaction between corpus and age was significant at alpha=.05. THIS IS A NEW RESULT, NEEDS WORK. Supporting this result visually, Figure 7 represents identification error rate as a function of age and corpus for individual children.
<!-- eb: what's with the 6 recordings with a 0% error? is it all non-speech? ac2eb not necessarily; if you do subset(py,child=="WAR_3528") you can see that this child has 4 files with a non-zero error rate, but by taking the median, these non-zero errors are not shown. The zeros do indeed arise in this one child because there is zero speech in all clips but 4 (column called total=0) ; same thing for BER_6035 BER_1618 BER_6035 SOD_3634 ROW_3486 although they have more non-silent clips, the zero still wins as a median-->
...
<!-- eb: above imiplies that no other within-corpus effects came out, is that true? -->
...
<!-- eb: how about adding "i.e., there were different quantities of CTC across corpora, but human CTC was not differentially predicted by system CTC" -->
...
Our first set of analyses tested overall accuracy, using established speech and talker segmentation metrics (false alarm rate, miss rate, confusion rate, and the composite identification error rate), and evaluated the pattern of errors in more detail, by assessing how LENA^®^ and human annotators agreed (precision and recall).  The median identification error rate was relatively high (`r round(median(py$identification.error.rate..,na.rm=T))`%), mainly due to a high miss rate (missing or excluding speech that was there; `r round(median(py$missed.detection..,na.rm=T))`%). The false alarm rate (identifying non-speech/silence as speech; `r round(median(py$false.alarm..,na.rm=T))`%) and confusion rate (identifying voice type; `r round(median(py$confusion..,na.rm=T))`%) were low. 

To put these numbers in context, the exact same analysis on inter-rater reliability data in the ACLEW project (which contains the four English corpora used here in addition to two non-English ones not studied here) reveals an identification error rate of 56%, which is somewhat lower, suggesting that LENA^®^-human departures are greater than human-human disagreement.  
...
<!-- #eb: i reworded, check to see if meaning is kept true to intent -->
\newpage
