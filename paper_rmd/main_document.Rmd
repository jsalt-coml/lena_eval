...
<!-- I find this last sentence confusing and seems to contradict the first part of the paragraph. Melanie -->
...
<!-- I think the references in the footnote are not being parsed correctly by rmd.- Melanie -->
...
As for LENA^TM^ word counts, the regions sampled for manual annotation were not guaranteed to perfectly align with LENA^TM^ segments, and human utterances need not align with them either. Of all LENA^TM^ segments overlapping with human vocalizations, 14% had only partial overlap with the annotations. To match LENA^TM^ AWCs with the annotated word counts, words from the partially overlapping LENA^TM^ utterances were included in proportion to the amount of overlap between the LENA^TM^ turn and the reference segment in question (e.g., if 10% of a LENA^TM^ segment  overlapped with a manually-annotated utterance, 10% of the total LENA^TM^ AWC estimate for that segment was included in the LENA^TM^ word count estimate for that utterance). 
...
```{r ctc-fig, ig.pos = "T!", fig.width=5.5, fig.height=5.5, fig.cap="Conversational Turn Counts according to LENA (x axis) and humans (y axis). Each point represents the CTC totaled within a clip. The solid line corresponds to a linear regression fit to data from all clips; the dashed line corresponds to an analysis excluding clips where both the human and LENA^TM^ said zero child-adult or adult-child turns"}
...
<!-- I think it is important to remind the reader here that the AWC results do not include the Tsimane data. - Melanie -->
...
```{r awc-fig, ig.pos = "T!", fig.width=5.5, fig.height=5.5, fig.cap="Adult Word Counts according to LENA (x axis) and humans (y axis). Each point represents the AWC totaled within a clip. The solid line corresponds to a linear regression fit to data from all clips; the dashed line corresponds to an analysis excluding clips where both the human and LENA^TM^ said there were no adult words."}
...
```{r der-fig, fig.cap="Diarization error rate as a function of corpus and child age (divided into quartiles). Each point represents the results averaged over all clips extracted from the data of one child. Colors indicate to which corpus that child belongs: BER in blue, L05 is green, SOD in black, TSI in gray, and WAR in purple "}
...
bychild=aggregate(py$diarization.error.rate..,list(py$child,py$cor,py$quartile),median,na.rm=T)
...
plot(bychild$x~bychild$jitter,ylim=range(0,115),pch=20,xlab="Age in months (quartiled)",xaxt="n",ylab="Diarization error rate",xlim=c(0.5,4.5),col=alpha(mycols[bychild$Group.2],.6))
...
<!-- For some reasons, this section header did not knit correctly for me. - Melanie -->
