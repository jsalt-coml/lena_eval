...
for(thiscor in levels(py$cor)){ 
 ...
 maxChiSq=max(maxChiSq,myANOVAs2[[thiscor]][,1][2])
if(thiscor!="SOD")  minp=min(minp,myANOVAs2[[thiscor]][,3][2])
}
...
mymodels<- list()
myANOVAs<- list()
for(thiscor in levels(as.factor(cvtc$cor))){ 
  #print(thiscor)
 mymodels[[thiscor]]<-lmer(gold_CV_count~lena_CV_count + (1|child), data=cvtc,subset=c(cor==thiscor))
