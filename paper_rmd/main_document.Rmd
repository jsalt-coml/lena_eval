...
library(psych) # INTRODUCES A BUG!!!
cohen.kappa(colapall)->ck
detach(psych)
```

Given results above suggesting that our coding of electronic may not have coincided with LENA^®^'s, and that "far" categories are inaccurate, in this analysis we only consider the following labels for LENA^®^: (all others are collapsed into an Other category); and the following labels for human annotators: (all others are collapsed into an Other category). This analysis revealed a Cohen Kappa estimated at K(`r ck$n.obs`) = `r ck$kappa` (confidence interval `r ck$kappa-ck$var.kappa*2`-`r ck$kappa+ck$var.kappa*2`), weighted Kappa estimated at K(`r ck$n.obs`) = `r ck$weighted.kappa` (confidence interval `r ck$weighted.kappa-ck$var.weighted*2`-`r ck$weighted.kappa+ck$var.weighted*2`).
