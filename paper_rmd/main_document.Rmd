...
```{r cf-dm,echo=F}
# Old version based on identification error rate.
# We switched to identification error rate
...
# }
# 
# allpy= rbind(pytot,pysil,pyfar,pynotv)
# 
# clean(allpy)->allpy 
# 
...
# medians=medians[c(4,2,3,1),]
# rownames(medians)<-c("Speakers","+ Overlap","+ Electronic","+ Far")

# [CHI,OCH,MAL,FEM,OVL] // ELE mapped to SIL
read.csv("../evaluations/gold_no_ele_lena_sil_no_tv_same/ider_report.csv")->pynotv
# [CHI,OCH,MAL,FEM,OVL,ELE] 
read.csv("../evaluations/gold_lena_sil_same/ider_report.csv")->pynotvnoovl
...
# 
# apa_table(medians,caption="False Alarm Rate (FAR), Miss Rate (MR), Confusion Rate, and total Identification Error Rate (IDER, sum of the medians of the other three categories), as a function of which categories are considered. Speakers indicates that only speaker categories are considers (all others are mapped onto Other); + Overlap that overlap in both human and LENA annotation were also scored; + Electronic that also electronic was scored; + Far that all far LENA categories were also scored.")
...
For simplicity's sake, for the subsequent re-analyses using different mappings from talkers to categories as bulleted above, we focus on only *medians* over all clips, capturing performance across all samples without being overly sensitive to outliers. In the second-best performing case, electronic voices in the human annotation are still mapped onto Other but Overlap is not, so that the human categories considered were CHI, FEM, MAL, OCH, and overlap; and the LENA^®^ categories considered were CHN, FAN, MAN, CXN, and OLN (with all "far" classes and TVN mapped onto Other). In this setting, although the miss rate is reduced when OLN is considered speech, overall performance degraded because OLN was not picking out the same regions as the overlapping speech found in the human annotations, leading to considerably higher false alarm and confusion rates.
...
read.table(paste0(thisdir,"/gold_key_child_voc_file_level.csv"),header=T,sep=" ")->gold


read.table(paste0(thisdir,"/lena_key_child_voc_file_level.csv"),header=T,sep=" ")->lena

#some cleaning to get the filenames to match up
gold$id=gold$filename
gold$id=gsub("BER_","",gold$id)
gold$id=gsub("WAR_","",gold$id)
gold$id=gsub("ROW_","",gold$id)
gold$id=gsub("SOD_","",gold$id)
gold$id=gsub("_0","_",gold$id)
lena$id=lena$filename
lena$id=gsub("_ ","_0",lena$id)

#join files by clip ID
cvtc=merge(lena,gold,by="id", all=T)
colnames(cvtc)<-gsub(".x","_lena",colnames(cvtc))
colnames(cvtc)<-gsub(".y","_gold",colnames(cvtc))

cor.cvc.all= cor.test(cvtc$CV_count_lena,cvtc$CV_count_gold)
cor.cvc.noZeros= cor.test(cvtc$CV_count_lena[cvtc$CV_count_lena>0 & cvtc$CV_count_gold>0],cvtc$CV_count_gold[cvtc$CV_count_lena>0 & cvtc$CV_count_gold>0])
...
myrange=range(cvtc[,c("CV_count_gold","CV_count_lena")],na.rm=T)
plot(CV_count_gold~CV_count_lena,data=cvtc,pch=20,col=alpha("black",.2),xlab="LENA CVC",ylab="Human CVC",xlim=myrange,ylim=myrange)
abline(lm(CV_count_gold~CV_count_lena,data=cvtc))
abline(lm(CV_count_gold~CV_count_lena,data=cvtc,subset=c(cvtc$CV_count_lena>0 & cvtc$CV_count_gold>0)),lty=2)
...
# error rate
aer=(cvtc$CV_count_lena-cvtc$CV_count_gold)
#summary(aer)


aer_no_zeros=(cvtc$CV_count_lena[cvtc$CV_count_lena>0 & cvtc$CV_count_gold > 0]-cvtc$CV_count_gold[cvtc$CV_count_lena>0 & cvtc$CV_count_gold > 0])

#sum(cvtc$CV_count_gold>0)
no_human_zeros=cvtc[ cvtc$CV_count_gold>0,]
rer=(no_human_zeros$CV_count_lena-no_human_zeros$CV_count_gold)/no_human_zeros$CV_count_gold*100
#summary(rer)

arer=abs(no_human_zeros$CV_count_lena-no_human_zeros$CV_count_gold)/no_human_zeros$CV_count_gold*100

aer_cvc <-(cvtc$CV_count_lena-cvtc$CV_count_gold)
aer_nozeros_cvc <-(cvtc$CV_count_lena[cvtc$CV_count_lena>0 & cvtc$CV_count_gold > 0]-cvtc$CV_count_gold[cvtc$CV_count_lena>0 & cvtc$CV_count_gold > 0])
...
myrange=range(cvtc[,c("CTC_count_gold","CTC_count_lena")],na.rm = T)
plot(CTC_count_gold~CTC_count_lena,data=cvtc,pch=20,col=alpha("blue",.2),xlab="LENA CTC",ylab="Human CTC",xlim=myrange,ylim=myrange)
abline(lm(CTC_count_gold~CTC_count_lena,data=cvtc))
abline(lm(CTC_count_gold~CTC_count_lena,data=cvtc,subset=c(cvtc$CTC_count_lena>0 & cvtc$CTC_count_gold>0)),lty=2)

```

```{r ctc-er, echo=F}

cor.ctc.all=cor.test(cvtc$CTC_count_lena,cvtc$CTC_count_gold)
cor.ctc.noZeros=cor.test(cvtc$CTC_count_lena[cvtc$CTC_count_lena>0 & cvtc$CTC_count_gold>0],cvtc$CTC_count_gold[cvtc$CTC_count_lena>0 & cvtc$CTC_count_gold>0])
...
# ER against human
aer=(cvtc$CTC_count_lena-cvtc$CTC_count_gold)
aer_no_zeros=(cvtc$CTC_count_lena[cvtc$CTC_count_lena>0 & cvtc$CTC_count_gold > 0]-cvtc$CTC_count_gold[cvtc$CTC_count_lena>0 & cvtc$CTC_count_gold > 0])

no_human_zeros=cvtc[ cvtc$CTC_count_gold>0,]
rer=(no_human_zeros$CTC_count_lena-no_human_zeros$CTC_count_gold)/no_human_zeros$CTC_count_gold*100

arer=abs(no_human_zeros$CTC_count_lena-no_human_zeros$CTC_count_gold)/no_human_zeros$CTC_count_gold*100

aer_ctc <-(cvtc$CTC_count_lena-cvtc$CTC_count_gold)
aer_nozeros_ctc <-(cvtc$CTC_count_lena[cvtc$CTC_count_lena>0 & cvtc$CTC_count_gold > 0]-cvtc$CTC_count_gold[cvtc$CTC_count_lena>0 & cvtc$CTC_count_gold > 0])
...
apa_table(allpearsons,caption="For each measure (CVC, CTC, AWC), N all stands for the total number of clips considered in the general correlation analysis and resulting Pearson r coefficient; N stands for the number of non-null clips (i.e., having some vocalizations, turns, and adult words respectively) as well as Pearson r in the analyses restricted to the non-null clips.",digits=c(0,2,0,2))
```
...
py=merge(py,age_id,by.x="child",by.y="child") 
...
```{r der-fig2, fig.cap="Identification error rate as a function of corpus and child age. Each point represents the results averaged over all clips extracted from the data of one child. Colors indicate to which corpus that child belongs: BER in blue, L05 is green, SOD in black, TSI in gray, and WAR in purple. "}
...
```{r cvc-age, echo=F}
cvtc$cor=substr(cvtc$filename_gold,1,3)
cvtc$cor[substr(cvtc$filename_gold,1,1)=="C"]<-"TSI"
cvtc$cor=factor(cvtc$cor)

cvtc$child=substr(cvtc$filename_gold,1,8)
#cvtc$child[cvtc$cor=="TSI"]<-substr(cvtc$filename_gold[cvtc$cor=="TSI"],1,3) #this line gives an error because there are no TSI files
#***** WARNING THIS NEEDS TO BE UNCOMMENTED !!!!! ******

merge(cvtc,age_id,by="child")->cvtc

mymodel<-lmer(CV_count_gold~CV_count_lena*age*cor + (1|child), data=cvtc)
#summary(mymodel)
...
for(thiscor in levels(cvtc$cor)){
  #print(thiscor)
 mymodels[[thiscor]]<-lmer(CV_count_gold~CV_count_lena*age + (1|child), data=cvtc,subset=c(cor==thiscor))
...
mymodel<-lmer(CTC_count_gold~CTC_count_lena*age*cor + (1|child), data=cvtc)
#summary(mymodel)
#Anova(mymodel,type="III")
