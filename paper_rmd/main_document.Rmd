...
For CVC, we fit a mixed model where CVC according to the human was predicted from CVC according to LENA^TM^, in interaction with corpus and age, as fixed factors, declaring child ID as random effect. An Analysis of Variance (type 2) found a triple interaction, suggesting that the predicted value of LENA^TM^ with respect to human CVC depended on both the corpus and the child age; and a two-way interaction between CVC by LENA^TM^ and corpus. To investigate these further, we fit a model where CVC according to the human was predicted from CVC according to LENA^TM^ in interaction with age (as fixed factors, with child ID as random) within each corpus separately. This revealed a significant interaction between LENA^TM^ CVC and age for BER (indicating that the predictive value of LENA^TM^ CVC increased with child age), whereas for the other four corpora this interaction was not significant, nor was the main effect of age, and only the LENA^TM^ CVC emerged as a significant predictor of variance in child vocalization counts derived from human annotation.[1] 
...
#there is a 2-way interaction between corpus and the predictive value of LENA^TM^'s counts with respect to the gold counts
...
For CTC, we fit a mixed model where CTC according to the human was predicted from CTC according to LENA^TM^, in interaction with corpus and age, as fixed factors, declaring child ID as random effect. An Analysis of Variance (type 2) found a two-way interaction between CTC by LENA^TM^ and corpus. To investigate this further, we fit the same regressions within each corpus separately.[2]  These follow-up analyses revealed that CTC by LENA^TM^ varied in its strength of prediction of human-tagged CTC across corpora  (
