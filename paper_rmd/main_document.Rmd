...
```{r marvin1-fig, fig.cap="Key child vocalizations and turn-takings related statistics aggregated across all children"}

all.stats = read.csv(paste0(thisdir,"../evaluations/key_child_voc_corpora_level.csv"), header=TRUE, sep = " ")
kable(all.stats) %>% kable_styling(latex_options="scale_down")
```
```{r marvin2-fig, fig.cap="Key child vocalizations and turn-takings related statistics at the child level"}
child.stats = read.csv(paste0(thisdir,"../evaluations/key_child_voc_child_level.csv"), header=TRUE, sep = " ")
kable(head(child.stats)) %>% kable_styling(latex_options="scale_down")
```

```{r marvin3-fig, fig.cap="Number of conversational turn taking as a function of the age"}
#rename human SIL as Other only for this graph, otherwise it'll break formulas elsewhere
scatterplot(child.stats$CTC_count~child.stats$age_mo_round,main="Number of conversational turn taking as a function of the age")
```
...
ber=read.table(paste0(thisdir,"../evaluations/BER","_cm.txt"),header=T)
row=read.table(paste0(thisdir,"../evaluations/ROW","_cm.txt"),header=T)
sod=read.table(paste0(thisdir,"../evaluations/SOD","_cm.txt"),header=T)
war=read.table(paste0(thisdir,"../evaluations/WAR","_cm.txt"),header=T)
all=read.table(paste0(thisdir,"../evaluations/all","_cm.txt"),header=T)
...
```{r DIAER,echo=F}
# Old version based on identification error rate. We switched to identification error rate
