...
In the human annotation, there is no class representing overlapping speech as such. For the sake of completeness and full comparison with the LENA^TM^ model, time where two or more different speech sources were active at the same time, according to the human annotators, have been mapped to the class "overlap" post hoc. This allows us to compare this Overlap class to LENA^TM^'s OLN (and, for the precision/recall analysis introduced next, OLF) by the LENA^TM^ model. Therefore, the confusion rate is computed based on the matches in Table 3.
...
kable(cor_tsi, "latex", caption = "Correspondances between LENA and our human annotation. *Electronic voices were only annotated in the ACLEW dataset. Although some Tsimane' families listen to the radio, radio speech was not annotated in the TSI corpus.", booktabs = T) %>%
...
Please remember that the overlap category is not defined the same way as the LENA^TM^ overlap category. For LENA^TM^, overlap between any two categories falls within overlap -- i.e., CHN+TV would be counted towards overlap as would FAN+FAN; whereas for us, only overlap between two talker categories (e.g., key child and female adult) counts as overlap. Similarly, the TVN is not equivalent to the electronic speech tag in the ACLEW coding.
...
For the AWC portion of this evaluation, we could only use transcriptions from the four ACLEW corpora, since the TSI corpus has not been transcribed (and thus lacks word counts). In contrast, annotators for the four ACLEW corpora were proficient in the language spoken in the daylong recording, and transcribed all adult speech based using canonical lexical forms (e.g. 'wanna', not 'want to') in keeping with minCHAT format [@macwhinney2017tools]. 

Reference adult word counts were determined by counting all unambiguously transcribed words spoken by adult talkers. This was achieved by first discarding all non-lexical transcript entries such as non-linguistic communicative sounds, paralinguistic markers, and markers indicating incomprehensible speech. In addition, all utterances from the key child and other children were omitted from the Adult Word Count. The remaining orthographic entries separated by whitespaces were then counted as gold standard target words for LENA^TM^ to detect. 

As for LENA^TM^ word counts, the regions sampled for manual annotation were not guaranteed to perfectly align with LENA^TM^ utterances. Of all LENA^TM^ utterances overlapping with the annotated speech, 14% had only partial overlap with the annotations. To match LENA^TM^ AWCs with the annotated word counts, words from the partially overlapping LENA^TM^ utterances were included in proportion to the amount of overlap between the LENA^TM^ turn and the reference segment in question (e.g., if 10% of a LENA^TM^-estimated utterance was overlapping with a manually-annotated utterance, 10% of the total LENA^TM^ AWC estimate for the given turn was included in the LENA^TM^ word count estimate for that reference segment). 
...
We thought the most informative decision would be to report on several of these settings, albeit briefly. We start with the situation that yields the best LENA^TM^ performance: Electronic voices in the gold annotation are mapped onto silence, so that the categories found in the human annotation are FEM, MAL, CHI, OCH, and overlap; in the LENA^TM^ annotation, only CHN, FAN, MAN, and CXN are considered speakers (with all far categories, TVN, and OLN all mapped onto silence). In this setting, LENA^TM^'s false alarm (i.e., saying that someone was speaking when they were not) had a median of `r round(median(py$false.alarm..,na.rm=T))`%, whereas the miss rate had a median of `r round(median(py$missed.detection..,na.rm=T))`%. 
...
These analyses shed light on the extent to which we can trust the LENA^TM^ tags to contain what the name indicates. We now move on to recall, which indicates a complementary perspective: how much of the original annotations were captured by LENA^TM^.
...
```{r cvc-fig, ig.pos = "T!", fig.width=5.5, fig.height=5.5, fig.cap="solid line corresponds to all clips, dashed clips corresponds to an analysis excluding clips where both the human and LENA^TM^ said zero child vocalizations."}
...
However, users need more: They also interpret the absolute number of vocalizations found by LENA^TM^. Therefore, it is important to also bear in mind the absolute error rate and the relative error rate. The absolute error rate tells us, given a LENA^TM^ estimate, how close the actual number may be. The relative error rate puts this number in relation to the actual number of vocalizations tagged by the human coder. For instance, imagine that we find that LENA^TM^ errs by 10 vocalizations according to the absolute error rate; this means that, on average across short clips like the ones used here, the numbers by LENA^TM^ would be off by 10 vocalizations. We may think this number is small; by using the relative error rate, we can check whether it is small relative to the actual number: An error of 10 vocalizations would seem less problematic if there are 100 vocalizations on average (LENA^TM^ would be just 10% off) than if there are 10 (LENA^TM^ would be doubling the number of vocalizations). 
...
```{r awc-fig, ig.pos = "T!", fig.width=5.5, fig.height=5.5, fig.cap="solid line corresponds to all clips, dashed clips corresponds to an analysis excluding clips where both the human and LENA^TM^ said zero AWC."}
...
For CTC, we fit a mixed model where CTC according to the human was predicted from CTC according to LENA^TM^, in interaction with corpus and age, as fixed factors; with child ID as random effect. An Analysis of Variance (type 2) found a two-way interaction between CTC by LENA^TM^ and corpus. To investigate this further, we fit the same regressions within each corpus separately.[2]  These follow-up analyses revealed that CTC by LENA^TM^ varied in its strength of prediction of human-tagged CTC across corpora  (
