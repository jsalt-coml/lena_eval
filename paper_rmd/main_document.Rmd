...
<!-- eb: i don't follow this last sentence, as written, reword? -->
<!-- eb: it's a bit confsing that absolute error rate isn't absolute value i.e. can be negative but for raer absolute does mean absolute value, pos only...are these the only potential standard terms? -->
...
We thought it most informative to report on several of these options, albeit briefly. We start with the situation that yields the best LENA^®^ performance: mapping electronic voices and overlap in the human annotation onto Other, so that the categories considered in the human annotation are FEM, MAL, CHI, OCH, alongside using only CHN, FAN, MAN, and CXN as speakers in the LENA^®^ annotation, (with all "far" categories, TVN, and OLN all mapped onto Other; see Tables 1 and 3). 
  Calculated in this way, LENA^®^'s *false alarm* rate (i.e., tagging a speech category when there was none) was  `r round(median(pytot$false.alarm..,na.rm=T))`% overall, with a mean across all clips of `r round(mean(py$false.alarm..,na.rm=T))`% and a median of `r round(median(py$false.alarm..,na.rm=T))`%. The *miss rate* was `r round(median(pytot$missed.detection..,na.rm=T))`% overall, with a mean across all clips of `r round(mean(py$missed.detection..,na.rm=T))`% and a median of `r round(median(py$missed.detection..,na.rm=T))`%. The *confusion rate* (i.e. confusing one talker category for another), as mentioned above, is only calculated for the correctly detected speech (i.e., not speech that was missed, which counts towards the miss rate, nor the speech that was falsely identified, which is part of false alarms). The confusion rate was very low, `r round(median(pytot$confusion..,na.rm=T))`% overall, with a mean across all clips of `r round(mean(py$confusion..,na.rm=T))`% and a median of `r round(median(py$confusion..,na.rm=T))`%. These three metrics (*false alarms*, *misses*, and *confusions*) are summed to yield the *identification error rate*. The overall identification error rate was  `r round(median(pytot$identification.error.rate..,na.rm=T))`%, with a mean across all clips of `r round(mean(py$identification.error.rate..,na.rm=T))`% and a median of `r round(median(py$identification.error.rate..,na.rm=T))`%.
...
medians$ider=rowSums(medians[2:4])
medians=medians[c(3,1,2),-1]
rownames(medians)<-c("Speakers","+ Overlap","+ Electronic")
```

```{r tabalts, echo=F}


colnames(medians)<-c("FAR","MR","CR","IDER")

apa_table(medians,caption="False Alarm Rate (FAR), Miss Rate (MR), Confusion Rate (CR), and total Identification Error Rate (IDER, sum of the medians of FAR, MR, and CR), as a function of which categories are considered. 'Speakers' indicates that only speaker categories are considers (all others are mapped onto SIL); '+ Overlap' that overlap in both human and LENA annotation were also scored; '+ Electronic' that also electronic was scored.",digits=0)
```


For simplicity's sake, for the subsequent re-analyses using different mappings from talkers to categories as bulleted above, we focus on only *medians* over all clips, capturing performance across all samples without being overly sensitive to outliers. In the second-best performing case, electronic voices in the human annotation are still mapped onto Other but Overlap is not, so that the human categories considered were CHI, FEM, MAL, OCH, and overlap; and the LENA^®^ categories considered were CHN, FAN, MAN, CXN, and OLN (with all "far" classes and TVN mapped onto Other). In this case, although the miss rate is reduced when OLN is considered speech, overall performance degraded because OLN was not picking out the same regions as the overlapping speech found in the human annotations, leading to considerably higher false alarm and confusion rates.

Next, we allowed the electronic voices segmented by human annotators, and LENA^®^'s TVN speaker categories to be included in the evaluation (rather than mapping them all to Other), such that the human categories considered were CHI, FEM, MAL, OCH, overlap, and electronic; and the LENA^®^ categories considered were CHN, FAN, MAN,  CXN, OLN, and TVN. Again, the miss rate drops slightly as we allow one more class to be considered speech, but this does not make up for the higher false alarm and confusion rate, likely  because TVN is not defined similarly to the electronic speech segmented by ACLEW annotators. 


Finally, we included the maximum possible number of categories. The human categories considered were still CHI, FEM, MAL, OCH, overlap, and electronic, alongside the fuller range of LENA^®^ categories: CHN, FAN, MAN, CXN, OLN, TVN, CHF, FAF, MAF, CXF, OLF, and TVF. Performance degrades considerable when all the "far" classes are treated as speech, leading to huge false alarm rates and increased confusion rates. 
<!-- eb: no numbers on this? probably also worth reminding or stating somewhere clearly what the possible range of values is for  IDER and every other measure actually -->
...
rownames(all)[rownames(all)=="SIL"]<-"Other"
...
rownames(all)[rownames(all)=="Other"]<-"SIL"
...
Precision for CHN comes in second place, at `r round(stall$pr[stall$LENA=="CHN" & stall$human=="CHI"])`%;
...
For the remaining two "near" speaker labels (MAN, CXN), recall averaged `r round(mean(stall$rec[(stall$LENA=="MAN" & stall$human=="MAL")|(stall$LENA=="CXN" & stall$human=="CXN")]))`%, meaning that less than a third of male adult and other child speech is being captured by LENA^®^. In fact, most of these speakers' contributions are being tagged by the LENA^®^ as OLN (mean across MAN and CXN `r round(mean(stall$rec[(stall$LENA=="OLN" & stall$human=="MAL")|(stall$LENA=="OLN" & stall$human=="CXN")]))`%) or TV (mean across MAN and CXN is `r round(mean(stall$rec[(stall$LENA=="TVN" & stall$human=="MAL")|(stall$LENA=="SIL" & stall$human=="CXN")]))`%), although the remaining sizable proportion of misses is actually distributed across many categories. 
...
<!-- eb: need to add a touch of interpretive info/reminder what all these DVs are for readers in this section; i can take this on -->
...
<!-- #eb: wrapping p.value in printp renders the right # of sigfigs for APA! -->
...
Again, we only considered "near" speaker categories in the turn count, and applied the same rule the LENA^®^ does, where a turn can be from the key child to an adult or vice versa, and should happen within 5 seconds to be counted. The association between clip-level LENA^®^ and human CTC was slightly weaker than that found for CVC (Figure 5):  the Pearson correlation between the two was r = `r round(cor.ctc.all$estimate,3)` (p = `r printp(cor.ctc.all$p.value)`) when all clips were taken into account, and r = `r round(cor.ctc.noZeros$estimate,3)` (p = `r printp(cor.ctc.noZeros$p.value)`)  when only clips with some child speech (i.e., excluding `r sum(cvtc$lena_CTC_count>0 & cvtc$gold_CTC_count>0)` clips with 0 counts in both LENA^®^ and human annotations) were considered.  
...
the mean was `r mean(aer_no_zeros)`% and the median was `r median(aer_no_zeros)`%. Again, *relative error rates* require non-null denominators, and we thus removed the `r sum(cvtc$gold_CTC_count==0)` clips in which the human annotator said there were no child-adult or adult-child turns at all. The mean relative error rate ranged from `r min(rer)`% to `r max(rer)`%, with a mean of `r mean(rer)`% and a median of `r median(rer)`%. Finally, the *absolute relative error* rate ranged from `r min(arer)`% to `r max(arer)`%, with a mean of `r mean(arer)`% and a median of `r median(arer)`%. For CTCs, as for CVCs, the LENA^®^ seems to be underestimating counts rather systematically, leading to counts that are on roughly a quarter smaller than they actually are.
...
apa_table(aller,caption="Mean, min, and max for each type of error rate for CVC, CTC, and AWC. Error rates are: AER (absolute error rate), AER-o (AER without clips with a zero count according to human or system analysis), RER (relative error rate), and ARER (absolute RER); RER and ARER exclude clips where the human count is zero.  ",digits=0)
#eb: should do something cleverer with nested column headers in e.g. stargazer or at least explain min/1/2/max/1/2.
```
...
<!-- eb: still need to add actual model tables to output or minimally SI -->
```{r lmer-dm, echo=F}
...
```{r der-fig2, fig.cap="Identification error rate as a function of corpus and child age. Each point represents the results averaged over all clips extracted from the data of one child. Color and shape indicates corpus: BER in blue circles, L05 is green triangles, SOD in black squares, TSI in gray pluses, and WAR in purple crossed squares."}
...
# plot(bychild$x~bychild$Group.3,ylim=range(0,115),pch=20,xlab="Age in months",ylab="Identification error rate",col=alpha(mycols[bychild$Group.2],.6))
# 
# 
# #legend
# xs=50
# ys=seq(115,95,-5)
# names(ys)<-names(mycols)
# for(cor in names(ys)) {
#   points(50,ys[cor],pch=20,col=mycols[cor])
#   text(54,ys[cor],cor)
#   }

#eb: would be nice to have diff shapes for each corpus; dunno how to do this in base r graphics, redid with ggplot, feel free to revert
bychild %>% 
...
<!-- eb: what's with the 6 recordings with a 0% error? is it all non-speech? -->
...
mymodels<- list()
myANOVAs<- list()
for(thiscor in levels(cvtc$cor)[-4]){ #!!!! removing TSI -- REMOVE THIS -4 WHEN TSI ARE FIXED!!!
  #print(thiscor)
