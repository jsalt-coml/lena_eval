...
#eb path change
abbreviations_table <- read.table("paper_rmd/figures/table_1.csv",header=F,sep="\t")
...
datasets_table <- read.csv("paper_rmd/figures/table_2.csv",header=T)
...
```{r read-cm,echo=F}
source("paper_rmd/readin_cm.R")
```

```{r DIAER,echo=F}

source("paper_rmd/readin_ider.R")
```
...
pynotv <- read.csv("evaluations/gold_no_ele_lena_sil_no_tv_same/ider_report.csv")->pynotv
# [CHI,OCH,MAL,FEM,OVL,ELE] 
#eb path change
#read.csv("../evaluations/gold_lena_sil_same/ider_report.csv")->pynotvnoovl
pynotvnoovl <- read.csv("evaluations/gold_lena_sil_same/ider_report.csv")
...
#eb path change
#read.table(paste0(thisdir,"/key_child_voc_file_level.csv"),header=T,sep=" ")->cvtc
cvtc <- read.table("evaluations/key_child_voc_file_level.csv",header=T,sep=" ")
...
#eb this subset appears to be all false & breaks the rmd so i'm commenting out this line
#abline(lm(gold_CTC_count~lena_CTC_count,data=cvtc,subset=c(cvtc$lena_CTC_count>0 & cvtc$gold_CTC_count>0)),lty=2)

```

```{r ctc-er, echo=F}
#eb: some funky stuff here preventing rendering; no quick fix that i see sorry
#eb: note that sd = 0
# cor.ctc.all=cor.test(cvtc$lena_CTC_count,cvtc$gold_CTC_count)
# #eb: Error in cor.test.default(cvtc$lena_CTC_count[cvtc$lena_CTC_count > 0 &  : 
...
```{r lmer-dm, echo=F}
#eb: was going to change path but i THINK this is all the same as in the readin_ider.R script
# so is just redundant with what's soruced in above, commenting out.
...
#REMOVE WHEN FILENAME=ZERO BUG IS FIXED 
cvtc=subset(cvtc,filename!=0)
cvtc$cor=factor(cvtc$cor)
